...
// Blinter v1.0.94 detailed format: "Line N: Message (CODE)"
...
function classifyMessage(message, severity) {
    if (!message) {
        return { classification: severity === 'info' ? 'Info' : 'General', isCritical: severity !== 'info' };
    }
    ...
    if (msg.includes('deprecated')) {
        return { classification: 'Deprecated', isCritical: severity !== 'info' };
    }
    ...
    return { classification: 'General', isCritical: severity !== 'info' };
}
...
function analyzeLine(line, options) {
    ...
    // Try detailed format first (Blinter v1.0.94+)
    ...
    if (detailed && detailed.groups) {
        ...
        let message = detailed.groups.message.trim();

        // Map severity from code prefix (E=error, W/SEC=warning, S/P=info)
        let severity = 'information';
        const codeUpper = code.toUpperCase();
        if (codeUpper.startsWith('E')) severity = 'error';
        else if (codeUpper.startsWith('W') || codeUpper.startsWith('SEC')) severity = 'warning';
        else if (codeUpper.startsWith('S') || codeUpper.startsWith('P')) severity = 'information';
...
function createIssue({ severity, message, filePath, lineNumber, code = undefined, variableIndex }) {
    const { classification, isCritical } = classifyMessage(message, severity);
